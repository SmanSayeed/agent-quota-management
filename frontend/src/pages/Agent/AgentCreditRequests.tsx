...
export default function AgentCreditRequests() {
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      // Update local balance since it was deducted
  ...
  const confirmApprove = () => {
    if (selectedRequest && approveAmount) {
      ...
      if (user?.creditBalance && amount > user.creditBalance) {
        toast.error('Insufficient credit balance to approve this amount');
        return;
      }
